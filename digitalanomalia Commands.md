# digitalanomalia Commands
TTS is disabled <br>


!Openminigames costs 10000 Units and lasts 6 minutes <br>
## Custom Commands <br>``` 

## Base Commands 
```js
!help<The Help Command | Catagories: Units, Minigames, Events, About, TTS. or get a full list here: github.com/MasterAirscrachDev/BeanBotFullCommands/blob/main/digitalanomalia%20Commands.md
!units<You Gain 1-3 Units Every Minute Your Active in Chat | Commands: !Units !GiveUnits, !UnitBoard, !FreeUnits, !Floor, !Steal
!giveunits<Give someone Units | !giveunits (@user) (amount)
!unitboard<View The Leaderboard
!freeunits<Get 100 Free Units once per stream
!floor<Check the floor for any {Program.config.currencies} | {Program.config.prefix}floor
!editunits<[STREAMER] Edit a users Units | !editunits (optional @user) (amount) | edit gold !editunits (@user) (amount) gold
!minigames<Minigames to play in chat | Commands: !OpenMinigames !Coinflip  !OneUps !Quickmath 
!openminigames<Unlocks the minigames for 6 minutes, can be stacked | !openminigames (costs 10.000 Units)
!coinflip<Flip a coin, if you bet you will get double your bet on correct guess | !coinflip (optional:[heads/tails] [amount]) betting is capped at 1000 Units
...
!playwithfire<Play a game with fire, if you win you will get 10x your bet, if you lose you get banished from chat for 2 minutes | !playwithfire (bet) betting is capped at 1000 Units
...
!buypadlock<buy a padlock to stop people stealing from you | !buypadlock (tier[1-3]) 1: 10% Units for 10, 2: 20% Units for 20M, 3: 30% Units for 30M and notification on break
...
!drop<collect a drop of 50 units | !drop
...
!buytts<buy TTS Tokens, -1 Units, 5% discount per token | !buyTTS (amount)
!tts<TTS is disabled
...
!goldenUnitblessing<activates the power of a golden Unit
!reload<[MOD] Reloads the config file and all custom commands | !reload
!prestige<Prestige your Units | !prestige (costs 1,000,000,000 Units)
